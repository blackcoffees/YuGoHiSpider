...
from util.image_util import save_image_local, rename_image
from util.tags_util import CARTLIMIT, MONSTERTYPE, ATTRIBUTE, LINK, \
    CARDTYPE, TYPE, ICON, RARITY, PACKAGE, logger


class CardSpider():
    def work(self):
        page = int(get_spider_now_page())
        while True:
            try:
                ...
                for card in json_cards.get("cards"):
                    ...
                    print chi_name
                    # 卡片是否存在
                    cart_exit = get_cart_by_chi_name(chi_name)
                    if cart_exit:
                        rename_image(cart_exit.get("chi_name"), str(cart_exit.get("id")))
                        continue
                    ...
                    if next_div_other_head:
                        ...
                        while True:
                            if next_div_other_head:
                                ...
                                print u"其他：%s：%s" % (page_head_text, next_div_other_value.text)
                                # 种族
                                ...
                                # 属性
                                    ...
                                    if tags_attribute:
                                        list_cart_detail.append(create_cart_detail(tags_attribute.get("id"),
                                                                                   tags_attribute.get("tags_eng_name")))
                                # 星级
                                ...
                                if page_head_text == u"罕见度":
                                    ...
                                    for page_rarity in list_page_rarity:
                                        ...
                                        if tags_rarity:
                                            list_cart_detail.append(create_cart_detail(tags_rarity.get("id"),
                                                                                       tags_rarity.get(
                                                                                           "tags_eng_name")))
                                # 下一行数据
                    ...
                    # 卡包
                    list_page_table = div_card_detail.find_all("table")
                    table_cart_package = None
                    for temp_page_table in list_page_table:
                        if temp_page_table.attrs.get("id") == u"pack_table_main":
                            table_cart_package = temp_page_table
                            break
                    if table_cart_package:
                        tr_list_cart_package = table_cart_package.find_all("tr")[1:]
                        for tr_cart_package in tr_list_cart_package:
                            if not tr_cart_package.text:
                                continue
                            # 新增卡包
                            pack_jpe_name = tr_cart_package.find("a").text
                            print u"卡包：%s" % pack_jpe_name
                            cart_package = get_cart_package_by_jpe_name(pack_jpe_name)
                            if not cart_package:
                                cart_package = CartPackage()
                                cart_package.jpe_name = pack_jpe_name
                                cart_package.eng_name = pack_jpe_name
                                cart_package.chi_name = pack_jpe_name
                                ...
                                page_pack_rarity = tr_cart_package.find_all("td")[2].text
                                tags_rarity = get_tags_rarity(page_pack_rarity)
                            ...
                            # 新增cart_detail
                            list_cart_detail.append(create_cart_detail(tags_package.get("id"), "Package"))
                    # 效果详细描述
                    ...
                    if div_effect_detail:
                        ...
                        if list_li_card_detail and len(list_li_card_detail) >= 2:
                    ...
                    cart.icon_url = save_image_local(icon_url, cart.chi_name)
                    cart_id = cart.save()
                    rename_image(cart.chi_name, str(cart_id))
                    # 保存cart_detail
                    for cart_detail in list_cart_detail:
                        cart_detail.cart_id = cart_id
                        cart_detail.save()
            except BaseException as e:
                logger.error("%s page:%s" % (cart.chi_name, page))
                logger.error(e)
                logger.error("\n")
            finally:
                if not json_cards.get("cards") or len(json_cards.get("cards")) == 0:
